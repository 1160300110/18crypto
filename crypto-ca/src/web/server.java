...
class ChatServer {
	...
	// 建立连接方法,最主要的就是创建ServerSocket对象，然后调用accept方法返回给一个Socket对象，后面就是明确Socket对象s调用方法getInputStream和getOutputStream方法的返回值分别作为new
	// DataInputStream和DataOutstream对象的参数
...
{
	// 下面是对窗口监听类中方法的重写,因为借口中的方法都是抽象的，所以使用时都需要重写，下面是java
	// api文档中窗口事件监听器WindowListener中的相关方法
